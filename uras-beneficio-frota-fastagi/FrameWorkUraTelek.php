...
function tracking_canal_ativo($canal, $ddr, $ticket, $indice)
{
    ...
    else
    {
        ...
        tracking($canal, $ddr, $ticket, $indice, 'FINALIZACAO', 'PERCURSO', 'CLIENTE DESLIGOU A CHAMADA DURANTE A URA');
...
function canal_ativo_pesquisa_satisfacao($canal, $ddr, $ticket, $indice)
{
    global $configs,$fastagi;
    $status=$fastagi->channel_status();

    if($status["data"] == 'Line is up')return true;
    if($status["data"]=="Channel is down, but reserved"){
        verbose("CANAL RESERVADO PARA O CLIENTE : ".$status["data"]);
        return true;
    }else{
        verbose("ENCERRANDO AGI POIS O CANAL DE ORIGEM NAO ESTA MAIS ATIVO, STATUS ATUAL DO CANAL :".$status["data"]);
