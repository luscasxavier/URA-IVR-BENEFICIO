    ...
    switch ($opcao) {
        ...
        case '9':            
            ...
            if($horaAtual >= '08:00' && $horaAtual <= '20:40'){
                ...
                $protocolo=protocolo_v2_v2($processId, '', '', $cartao, $flagEnt, $origemSolicitacao, '', $origem, '', $uniqueid);
...
function deseja_ser_cliente($uniqueid, $origem){
    ...
    $identificador= coleta_dados2($canal, $ddr, $ticket, $indice, "FroCli/05",18);
    ...
    //28
    $indice++;
    tracking($canal, $ddr, $ticket, $indice, 'DESEJA SER CLIENTE VALECARD', 'CPF/CNPJ', $identificador);

    if(strlen($identificador)>=11 && strlen($identificador)<=14){
        verbose("CNPJ/CPF DIGITADO CORRETAMENTE");
        inicializa_ambiente_novo_menu();
        checa_contrato($uniqueid, $origem, $identificador);
    }else{
        verbose("CLIENTE INFORMOU DE FORMA INCORRETA");
        if(retentar_dado_invalido("PRINCIPAL","uraBeneCli/03","OPCAO INVALIDA")){
            //57
            $indice++;
            tracking($canal, $ddr, $ticket, $indice, 'DESEJA SER CLIENTE VALECARD', 'PERCURSO', 'OPCAO INVALIDA(INFORME O CPF/CNPJ)');

            deseja_ser_cliente($uniqueid, $origem);
        }else{
            //58
            $indice++;
            tracking($canal, $ddr, $ticket, $indice, 'DESEJA SER CLIENTE VALECARD', 'PERCURSO', 'TENTATIVAS EXCEDIDAS(INFORME O CPF/CNPJ)');

            encerra_com_tracking($canal, $ddr, $ticket, $indice, "DESEJA SER CLIENTE VALECARD","FroCli/04","OPCAO INVALIDA");  
        }
    }
...
function checa_contrato($uniqueid, $origem, $identificador){
    global $canal, $ddr, $ticket, $indice, $horaAtual;

    $validaIdent= valida_cli_frota($uniqueid, $origem, $identificador);
    
    $listProd= $validaIdent->{'listaProdutos'};
    $a='0';
    $audio='';
    foreach ($listProd as $nomeObjeto => $valorObjeto) {
        if($valorObjeto!=""){
            verbose("CONTRATO ENCONTRADO: ".$nomeObjeto);
            $audio.="FroCli/".$nomeObjeto.'&';
            $a++;
        }
    }

    verbose("AUDIOOOOOOOOOOOOOOOOOOOOOOO : ".$audio);

    $audio= rtrim($audio, '&');
    
    if($audio){

        verbose("AAAAAAAAAAAAAAAAAA: ".$a);

        if($a=='1'){
            $audio= "FroUsu/Novo2Sing&".$audio."&FroUsu/Novo3Sing";
        }else{
            $audio= "FroUsu/Novo2&".$audio."&FroUsu/Novo3";
        }
        //$audio= "FroUsu/Novo2&".$audio."&FroUsu/Novo3";
        ...
        verbose("AUDIO A SER REPRODUZIDO: ".$audio);
        ...
        if($opcao==1){
            ...
            inicializa_ambiente_novo_menu();
            deseja_ser_cliente2($uniqueid, $origem, $identificador);
        }
    }else{
...
function deseja_ser_cliente2($uniqueid, $origem, $identificador){
    global $canal, $ddr, $ticket, $indice, $horaAtual;
    tracking_canal_ativo($canal, $ddr, $ticket, $indice);
    ...
    if(strlen($identificador)==11){
        ...
        if($lgpd=='1'){
            ...
            inicializa_ambiente_novo_menu();
            informa_telefone($uniqueid, $origem, $identificador);
...
function informa_telefone($uniqueid, $origem, $identificador){
    global $canal, $ddr, $ticket, $indice, $horaAtual;
    ...
    $protocolo=api_ucc_protocolo_v2($categoria, $identificador, '', $flagEnt, $origemSolicitacao, $subCat, $origem, $telefone, $uniqueid);
    verbose("PROTOCOLO GERADO : ".$protocolo);
/*    $prot=protocolo_mensagem($processId, $categoria, $identificador, '', $flagEnt, $origemSolicitacao, $subCat, $origem, $telefone, $uniqueid);
    ...
    if(strlen($mensagem)>=0 && srtlen($protocolo)>=5){
...
*/
    $indice++;
    tracking($canal, $ddr, $ticket, $indice, 'DESEJA SER CLIENTE VALECARD', 'PROTOCOLO', $protocolo);
...
function validar_dados_fro_cli($uniqueid, $origem){
    ...
    if($identificador == '-1'){hangup();break;exit;}
    tracking_canal_ativo($canal, $ddr, $ticket, $indice);
    ...
    if(strlen($identificador)<11 && $identificador!='TIMEOUT'){
        ...
        if($validaIdent->{'codClienteValidado'}=="S" && $validaIdent->{'clienteBeneficio'}=="N"){
            ...
            if(retentar_dado_invalido("validar_dados","FroCli/08","CODIGO NAO VALIDADO PELA API"))validar_dados_fro_cli($uniqueid, $origem);
                ...
                $flagEnt='cliente';
                $origemSolicitacao='URA_FROTA_CLIENTE';
                $protocolo=protocolo_v2_v2($processId, '', $identificador, '', $flagEnt, $origemSolicitacao, '', $origem, '', $uniqueid);
...
function validar_dados_cnpjcpf($uniqueid, $origem, $identificador, $validaIdent){
    ...
    /*
        ...
        $aquisicaoInsumos= $listaCartoes->{'aquisicaoInsumos'};
        $primePosto= $listaCartoes->{'primePosto'};
        $frotaCredito= $listaCartoes->{'frotaCredito'};
        $pedagioEletronico=$listaCartoes->{'pedagioEletronico'};
        $telemetria=$listaCartoes->{'telemetria'};
        $primePostoGarantido=$listaCartoes->{'primePostoGarantido'};
        $cartaoLogistica=$listaCartoes->{'cartaoLogistica'};
    ...
    */
    ...
    foreach ($listaCartoes as $key => $value) {
        if ($value != '') {
            verbose("ID DO CARTAO ".$key. ": ".$value);
            $ContratoAudio .= "FroCli/DIGITE".$fat."&FroCli/".$key."&";
            $fat++;

            $nomeProduto[$indice] = $key;
            $produtosNumerados[$indice] = $value;
            $indice++;
        }
    }

    $ContratoAudio = rtrim($ContratoAudio, '&');
    /*

        // A PROVA DE QUE PROGRAMACAO E UMA COISA DE DOIDO

        if($aquisicaoInsumos!='' && !is_null($aquisicaoInsumos)){
            verbose("CARTAO ID DE aquisicaoInsumos : ".$aquisicaoInsumos);
            $ContratoAudio.="FroCli/DIGITE".$fat."&FroCli/aquisicaoInsumos&";
            $fat++;

        }elseif($primePosto!='' && !is_null($primePosto)){
            verbose("CARTAO ID DE primePosto : ".$primePosto);
            $ContratoAudio.="FroCli/DIGITE".$fat."&FroCli/primePosto&";
            $fat++;

        }elseif($frotaCredito!='' && !is_null($frotaCredito)){
            verbose("CARTAO ID DE frotaCredito : ".$frotaCredito);
            $ContratoAudio.="FroCli/DIGITE".$fat."&FroCli/frotaCredito&";
            $fat++;

        }elseif($pedagioEletronico!='' && !is_null($pedagioEletronico)){
            verbose("CARTAO ID DE pedagioEletronico : ".$pedagioEletronico);
            $ContratoAudio.="FroCli/DIGITE".$fat."&FroCli/pedagioEletronico&";
            $fat++;

        }elseif($telemetria!='' && !is_null($telemetria)){
            verbose("CARTAO ID DE TELEMETRIA : ".$telemetria);
            $ContratoAudio.="FroCli/DIGITE".$fat."&FroCli/telemetria&";
            $fat++;

        }elseif($primePostoGarantido!='' && !is_null($primePostoGarantido)){
            verbose("CARTAO ID DE primePostoGarantido : ".$primePostoGarantido);
            $ContratoAudio.="FroCli/DIGITE".$fat."&FroCli/primePostoGarantido&";
            $fat++;

        }elseif($cartaoLogistica!='' && !is_null($cartaoLogistica)){
            verbose("CARTAO ID DE cartaoLogistica : ".$cartaoLogistica);
            $ContratoAudio.="FroCli/DIGITE".$fat."&FroCli/cartaoLogistica&";
            $fat++;

        }elseif($abastecimento!='' && !is_null($abastecimento)){
            verbose("CARTAO ID DE abastecimento : ".$abastecimento);
            $ContratoAudio.="FroCli/DIGITE".$fat."&FroCli/abastecimento&";
            $fat++;

        }elseif($manutencao!='' && !is_null($manutencao)){
            verbose("CARTAO ID DE manutencao : ".$manutencao);
            $ContratoAudio.="FroCli/DIGITE".$fat."&FroCli/manutencao&";
            $fat++;
    ...
    */
    ...
    if (isset($produtosNumerados[$opcao]) && $opcao <= $fat) {
        ...
        $valorSelecionado = $produtosNumerados[$opcao];
        $produto= $nomeProduto[$opcao];
        verbose("CONTRATO DO PRODUTO ".$produto." SELECIONADO: " . $valorSelecionado);
